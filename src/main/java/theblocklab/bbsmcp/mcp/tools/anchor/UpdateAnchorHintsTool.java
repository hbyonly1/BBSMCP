...
public class UpdateAnchorHintsTool extends MCPTool {
  ...
  public MCPToolResponse execute(JsonObject arguments, MinecraftServer server) {
    ...
    try {
      // 可选：清空旧 hints
      if (clearExisting) {
        AnchorManagerAPI.INSTANCE.clearCameraHints(player, anchorId);
      }
      ...
      for (JsonElement elem : hints) {
        ...
        AnchorManagerAPI.INSTANCE.addCameraHint(player, anchorId, hint);
        added++;
      }
      ...
      if (arguments.has("enrich_description")) {
        ...
        String anchorData = AnchorManagerAPI.INSTANCE.getAnchorJson(anchorId);
        if (anchorData != null) {
          ...
          AnchorManagerAPI.INSTANCE.update(player, anchorId, null, newDesc, null);
        }
      }
